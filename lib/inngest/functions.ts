    ...
    async ({ event, step }) => {
        ...
        await step.run("send-welcome-email", async () => {
            ...
            const introText = (part && "text" in part ? part.text : null) ||
                "Thanks for joining Signalist. You now have the tools to track the market and make smart moves.";
...
export const sendDailyNewsSummary = inngest.createFunction(
    ...
    [ { event: "app/send.daily.news" }, { cron: "0 12 * * *"} ], // min hour dayOfMonth month dayOfWeek
    async ({ step }) => {
